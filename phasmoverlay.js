...
const getUserLevelFromData = (data) => {
  ...
  for (let i = 0; i < badges.length; i++) {
    ...
    } else if (badges[i].type === "subscriber") {
      badgeLevel = PERMISSION_SUB;
...
const modOrSubPermission = (configuration) => {
  return configuration.allowSUBS ? PERMISSION_SUB : PERMISSION_MOD;
};
...
window.addEventListener("onWidgetLoad", function (obj) {
  ...
  // Sets up all the commands for the widget for modOrVip
  config.commands = {
  ...
  // Sets up all the commands for the widget for modOrSub
config.commands = {
    [fieldData["resetCommand"]]: (data) => {
      runCommandWithPermission(modOrSubPermission(config), data, _resetGhost, [
        data.text,
        userState,
      ]);
    },
    ...
    [fieldData["locationDiffCommand"]]: (data) => {
      runCommandWithPermission(modOrSubPermission(config), data, _setDiffName, [
        data.text,
        userState,
      ]);
    },
    [fieldData["bonerCommand"]]: (data) => {
      runCommandWithPermission(modOrSubPermission(config), data, _toggleBoner, [
        userState,
      ]);
    },
    [fieldData["ouijaCommand"]]: (data) => {
      runCommandWithPermission(modOrSubPermission(config), data, _toggleOuija, [
        userState,
      ]);
    },
    [fieldData["emfCommand"]]: (data) => {
      runCommandWithPermission(modOrSubPermission(config), data, _toggleEMF, [
        userState,
        config,
      ]);
    },
    ...
    [fieldData["orbsCommand"]]: (data) => {
      runCommandWithPermission(modOrSubPermission(config), data, _toggleOrbs, [
        userState,
        config,
      ]);
    },
  ...
  config.allowSUBS = fieldData["allowSUBS"] === "yes" ? true : false;
...
const _toggleSUBAccessibility = (canUseSUB) => {
  toggleSUBAccessibility(canUseSUB);
};
...
const toggleSUBAccessibility = (canUseSUB) => {
  if (canUseSUB !== undefined && canUseSUB !== null) {
    config.allowSUBS = canUseSUB;
  } else {
    config.allowSUBS = !config.allowSUBS;
  }
};
